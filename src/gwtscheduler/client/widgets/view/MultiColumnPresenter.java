...
//  }
  ...
  public void bindDisplay(final Display display) {
    ...
    // TODO: {lazo} added for testing
    eventBus.addHandler(DropEvent.TYPE, new DropHandler(){
      @Override
      public void onDrop(DropEvent event) {
        int mouseX = event.getMouseX();
        int mouseY = event.getMouseY();

        int columns = getColNum();
        int rows = getRowNum();
        ...
        if(height > 0 && width > 0){
          int colPos = (mouseX / (width / columns));
          int rowPos = (mouseY / (height / rows))-3;
          int[] pos = new int[] {rowPos, colPos};

          display.addTicket(event.getWidget(), pos);
        }
      }
    });
  }
